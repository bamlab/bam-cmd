...
Config.prototype.runDeploy = function(options) {
  var userDeploy = new Callable(this.config, 'deploy', [options]);
  var pluginDeploy = new Callable(null, 'runDeploy', [options, true]);

  return this.runUserMethod(userDeploy, pluginDeploy, true)
    .then(function () {
      if (! this.isPlugin()) {
        return this.runPostDeploy(options);
      }
    }.bind(this));
};
 ...
 * @returns {Promise}
 */
Config.prototype.runPostDeploy = function(options) {
  var postDeploy = new Callable(this.config, 'postDeploy', [options]);
  var pluginPostDeploy = new Callable(null, 'runPostDeploy', [options]);

  return this.runPluginsMethod(pluginPostDeploy, true)
    .then(function() {
      return this.runUserMethod(postDeploy, null, false);
    }.bind(this));
};
