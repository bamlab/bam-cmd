  ...
  return bamCmd.renameProjectDir(initialDir, config)
    ...
    .then(function() {
      ...
      if (dontInstallLinked) {
        ...
        promises.push(this.fetchLinkedRepos(config, dest));
      }
    ...
    .then(function(datas) {
      var promises = [];
      _.forEach(datas[0], function(linkedConfig) {
        var promise = linkedConfig.runPostInstall(options);
        promises.push(promise);
      });

      var mainPromise = config.runPostInstall(options);
