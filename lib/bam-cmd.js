 ...
 *
 * Rename the directory, fetch linked repos and run user scripts
 *
 * @param {String} initialDir Directory where the project is installed
 * @param {Object} config Configuration of the project
 * @param {boolean} dontInstallLinked true to don't install linked projects
 *
 * @return {Promise}
 */
bamCmd.setupProject = function(initialDir, config, dontInstallLinked) {
  var finalDir;
  ...
  return bamCmd.renameProjectDir(initialDir, config)
    .then(function(renamedDir) {
      ...
      return this.fetchLinkedRepos(config, dest);
    }.bind(this))
...
bamCmd.installProject = function(name, dest, shouldHaveBam, dontInstallLinked) {
  ...
  return this.cloneRepository(name, null, dest)
    ...
    .then(function(config) {
      return this.setupProject(clonedDir, config, dontInstallLinked);
